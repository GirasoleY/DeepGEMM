...
def parse_args():
    ...
    if not options.payload_per_mode or not options.transitions_per_mode:
        raise ValueError("dispatch sweep requires payload and fallback validation in every mode")
    if options.schedule != [0, 1, 0]:
        raise ValueError("dispatch sweep requires the full0,1,0 control/candidate/control schedule")
    return options, args


def prepare_sweep_environment():
    common.prepare_sweep_environment()
    os.environ[COMBINE_ENV] = "1"
...
def _payload_with_control(harness, mode, torch, dist):
    ...
    def compare(harness, torch, dist):
        ...
        errors = [f"payload expert assignments {name} differ from dispatch0 output"
                  for name, value in current.items()
                  if not torch.equal(value.view(torch.int16), baseline[name].view(torch.int16))]
        accuracy._collective_transition_check("dispatch-overlap/payload-control", errors, dist)
    ...
    result["same_context_dispatch0_per_epoch_bitwise"] = bool(mode and result["enabled"])
    return result
...
def _check_imbalance_output(harness, case, reference, expected, label, torch, dist):
    ...
    if reference is not None and not torch.equal(harness.output.view(torch.int16), reference.view(torch.int16)):
        errors.append("output differs bitwise from same-context dispatch0 graph")
    try:
...
def _fixed_shape_imbalance(harness, graph, mode, torch, dist):
    ...
    try:
        ...
        for epoch in dict.fromkeys(epochs):
            ...
            for name, case in cases.items():
                ...
                label = f"dispatch-overlap/mode{mode}/fixed-T/control/epoch{epoch}/{name}"
        ...
        for epoch in epochs:
            ...
            for step, name in enumerate(IMBALANCE_SEQUENCE):
                ...
                graph.replay()
                torch.cuda.synchronize()
                label = f"dispatch-overlap/mode{mode}/fixed-T/epoch{epoch}/{step}/{name}"
        ...
        common._check_identity(harness, initial, "dispatch-overlap/fixed-T-identity", dist)
    finally:
    ...
    return {"enabled": True, "timed": False, "single_candidate_graph": True,
            ...
            "payload_epochs": epochs, "same_context_dispatch0_bitwise": True,
...
def _drift(records):
    ...
    adapted = [{"single_combine_context": record["dispatch_overlap"], "timing": record["timing"]}
...
def main():
    ...
    def sweep(harness, snapshots, torch, dist):
        ...
        if any(value != configuration for value in gathered):
            errors.append("dispatch sweep configuration/source/environment differs across ranks")
        ...
        accuracy._collective_transition_check("dispatch-overlap/preflight-before-candidate", errors, dist)
        ...
        for entry, mode in enumerate(options.schedule):
            ...
            with patch.dict(os.environ, {MODE_ENV: str(mode)}):
                try:
                    label = f"entry{entry}/dispatch_overlap{mode}"
                    if dist.get_rank() == 0:
                        print("MEGAMOE_DISPATCH_OVERLAP_STAGE=" + label + "/capture", flush=True)
                    ...
                    checks["same_context_dispatch0_bitwise"] = checks.pop("same_context_mode0_bitwise")
                    ...
                    imbalance = _fixed_shape_imbalance(harness, graph, mode, torch, dist)
                    common._retire_graphs(graphs)
                    graph = None
                    payload = _payload_with_control(harness, mode, torch, dist)
                    ...
                    records.append({"entry_index": entry, "dispatch_overlap": mode,
                        "dispatch_contract": dispatch_contract(mode),
                    ...
                    if dist.get_rank() == 0:
                        print("MEGAMOE_DISPATCH_OVERLAP_PENDING_JSON=" + json.dumps({
                            "status": "pending_remaining_accuracy_and_teardown", "entry": entry,
                            "dispatch_overlap": mode,
        ...
        state["result"] = {
            "schema": "megamoe-dispatch-overlap-sweep-v1", "configuration": configuration,
            ...
            "modes": records, "repeated_dispatch0_drift": _drift(records),
        }
        accuracy._collective_transition_check("dispatch-overlap/source-unchanged",
            [] if source_fingerprint() == provenance else ["fingerprinted source changed during sweep"], dist)
    ...
    if int(os.environ["RANK"]) == 0:
        ...
        with options.output.open("x") as output:
            ...
            output.write("\n")
        print("MEGAMOE_DISPATCH_OVERLAP_SWEEP_JSON=" + json.dumps({
