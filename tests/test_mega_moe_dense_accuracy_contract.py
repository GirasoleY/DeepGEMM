...
class TestDenseAccuracyPolicy(unittest.TestCase):
    ...
    def test_combine_preflight_and_labels_precede_allocation_without_runtime_claim(self):
        ...
        self.assertIn("each peer independently selects", source)
        self.assertIn("peer_parallel_ready_coalesced_spans_then_late_header", source)
        self.assertIn("warp_parallel_readiness_peer_independent_bounded_ready_coalescing", source)
        self.assertIn("fixed cap8", source)
        self.assertIn('"ready_batch_max_experts": 8', source)
        self.assertIn('"ready_batch_cap_is_compile_time_constant": True', source)
        self.assertIn('"frozen_ready_snapshot_no_fill_wait": True', source)
        self.assertIn('"issuer_acquires_every_batched_expert": True', source)
        self.assertIn("not early readiness", source)
        self.assertIn('"saved_source_nonempty_mask_uint32": 8 * 2', source)
        self.assertIn('"tail_bytes": 2304', source)
        self.assertIn('"direct_reducer": {', source)
        self.assertIn('"address_preparation": "lane_local_token_row_pointer_preload_before_chunk_loop"', source)
        self.assertIn('"full_warp_pointer_gather_before_elected_issuer": True', source)
        self.assertIn('"metadata_resolution_passes_per_active_remote_assignment_per_token": 1', source)
        self.assertIn('"policy_not_device_observation": True', source)
        self.assertIn('"additional_source_local_ordinal_bytes": 3072', source)
        self.assertIn('"required_scratch_extent_bytes": 62720', source)
        self.assertIn('"added_barriers": 0', source)
        self.assertIn("one_async_global_proxy_fence_per_epilogue_thread_before_reduction", source)
        self.assertIn("existing_second_handoff_deferred_until_all_local_packet_reads_complete", source)
        self.assertIn("original_ascending_topk_slot_fp32_then_bf16", source)
        self.assertIn('"required_scratch_bytes": 59648', source)
